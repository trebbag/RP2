...
import { deadLetterSummary, listDispatchJobs, markDispatchJobDeadLetter, processDueDispatchJobs, replayDispatchJob } from "../services/dispatchService.js";
...
function resolveBucketIndex(date, startMs, bucketMs, bucketCount) {
    if (!date)
        return null;
...
function buildObservabilityTrendPoints(windowMinutes, requestedBucketMinutes) {
...
adminRoutes.post("/dispatch/:jobId/dead-letter", requireRole(["ADMIN"]), async (req, res, next) => {
    try {
        ...
        const deadLettered = await markDispatchJobDeadLetter(req.params.jobId, authReq.user.orgId, payload.reason, authReq.user.id);
        if (!deadLettered) {
            res.status(404).json({ error: "Dispatch job not found" });
            return;
        }
...
adminRoutes.get("/dispatch/sandbox-readiness", requireRole(["ADMIN"]), async (req, res, next) => {
    try {
        const authConfigured = env.DISPATCH_AUTH_MODE === "NONE" ||
...
adminRoutes.get("/observability/summary", requireRole(["ADMIN"]), async (req, res, next) => {
    try {
        ...
        const suggestionAcceptanceRate = suggestionDecisionCount > 0 ? Number((suggestionAcceptedCount / suggestionDecisionCount).toFixed(3)) : 0;
        ...
        const complianceFalsePositiveRate = complianceReviewedCount > 0 ? Number((complianceDismissedCount / complianceReviewedCount).toFixed(3)) : 0;
        const suggestionAcceptanceLow = suggestionDecisionCount >= env.AI_SUGGESTION_ACCEPTANCE_ALERT_MIN_DECISIONS &&
            suggestionAcceptanceRate < env.AI_SUGGESTION_ACCEPTANCE_ALERT_THRESHOLD;
        const transcriptCorrectionHigh = sttSegmentCount >= env.AI_TRANSCRIPT_CORRECTION_ALERT_MIN_SEGMENTS &&
            transcriptCorrectionRate > env.AI_TRANSCRIPT_CORRECTION_ALERT_THRESHOLD;
        const complianceFalsePositiveHigh = complianceReviewedCount >= env.AI_COMPLIANCE_FALSE_POSITIVE_ALERT_MIN_REVIEWED &&
...
adminRoutes.get("/observability/trends", requireRole(["ADMIN"]), async (req, res, next) => {
    try {
        ...
        const [dispatchRows, sttIngestRows, authFailureRows, suggestionDecisionRows, transcriptCorrectionRows, transcriptSegmentRows, complianceDismissedRows, complianceResolvedRows] = await Promise.all([
