...
function mapActionToSelection(actionType) {
    switch (actionType) {
        case "keep":
            return "KEEP";
        case "remove":
...
async function resolvePromptProfile(req) {
...
wizardRoutes.get("/:encounterId/state", async (req, res, next) => {
    try {
        ...
        let suggestedStep = 1;
        if (completedSteps.has("STEP1_CODE_REVIEW"))
            suggestedStep = 2;
        if (completedSteps.has("STEP2_SUGGESTION_REVIEW"))
            suggestedStep = 3;
        if (completedSteps.has("STEP3_COMPOSE"))
            suggestedStep = 4;
        if (completedSteps.has("STEP4_COMPARE_EDIT"))
            suggestedStep = 5;
        if (completedSteps.has("STEP5_BILLING_ATTEST"))
            suggestedStep = 6;
        ...
        if (selectedCodePayload.code && selectedCodePayload.codeType && selectedCodePayload.category) {
            await prisma.codeSelection.create({
                data: {
                    ...
                    category: selectedCodePayload.category,
...
//# sourceMappingURL=wizardRoutes.js.map