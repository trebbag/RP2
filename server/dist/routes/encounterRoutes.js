...
async function buildEncounterTranscriptQuality(orgId, encounterId) {
    ...
    return buildTranscriptQualityReport(segments.map((segment) => ({
        id: segment.id,
...
encounterRoutes.get("/:encounterId/transcript/stream", requireRole(["ADMIN", "CLINICIAN"]), async (req, res, next) => {
    try {
        ...
        res.flushHeaders?.();
        const queryClientId = typeof req.query.clientId === "string" && req.query.clientId.trim().length > 0
...
encounterRoutes.post("/:encounterId/transcript/audio", requireRole(["ADMIN", "CLINICIAN"]), transcriptUpload.single("audio"), async (req, res, next) => {
...
//# sourceMappingURL=encounterRoutes.js.map