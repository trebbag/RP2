...
const normalizedEnv = {
    ...
    MFA_REQUIRED: parsedEnv.MFA_REQUIRED ?? (parsedEnv.NODE_ENV === "production" && resolvedAuthMode === "local" ? true : false),
...
if (normalizedEnv.AUTH_MODE === "oidc") {
    const missing = [];
    if (!normalizedEnv.OIDC_ISSUER_URL)
        missing.push("OIDC_ISSUER_URL");
    if (!normalizedEnv.OIDC_CLIENT_ID)
        missing.push("OIDC_CLIENT_ID");
    if (!normalizedEnv.OIDC_CLIENT_SECRET)
        missing.push("OIDC_CLIENT_SECRET");
    if (!normalizedEnv.OIDC_REDIRECT_URI)
