...
import { defaultUserSettings, normalizeUserSettingsPayload } from "./settingsService.js";
...
function extractTaskOverrides(raw, task) {
    if (!raw.trim())
        return [];
    try {
        const parsed = JSON.parse(raw);
        const taskMap = (parsed.task_instructions || parsed.taskInstructions || parsed.tasks);
...
function stringifyContextBlock(runtimeContext) {
...
function truncateForPrompt(value, maxChars = OVERRIDE_TEXT_LIMIT) {
...
export async function loadPromptProfileForUser(userId) {
    if (!userId) {
...
export function buildPromptBundle(input) {
    ...
    sections.push("", "Guardrails:", "- only use evidence present in the provided input payload.", "- keep output deterministic and machine-parseable.", "- avoid placeholders like TBD/unknown unless truly missing from source.");
...
//# sourceMappingURL=promptBuilderService.js.map