...
export const SUPPORTED_PAYER_MODELS = ["MEDICARE", "AETNA_PPO", "BCBS_PPO", "SELF_PAY"];
...
const DEFAULT_BILLING_SCHEDULE_PACK = {
...
function resolveScheduleConfigPath() {
...
function loadBillingSchedulePack() {
...
function resolveWritableSchedulePath() {
    const runtimePathOverride = process.env.BILLING_SCHEDULES_PATH?.trim();
...
function normalizePayerModel(model) {
    if (!model)
        return "MEDICARE";
...
export function getBillingSchedulePack() {
    return loadBillingSchedulePack();
}
export function saveBillingSchedulePack(input) {
...
export function calculateBillingEstimate(input) {
    ...
    const allowedAmountCents = selectedCptCodes.reduce((sum, code) => sum + (schedule.cptRatesCents[code] ?? schedule.defaultRateCents), 0);
