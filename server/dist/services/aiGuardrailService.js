...
function resolveFallbackSpeaker(speakerHint, preferredSpeakers) {
    if (speakerHint && speakerHint.trim())
        return speakerHint.trim();
    if (preferredSpeakers && preferredSpeakers.length > 0)
        return preferredSpeakers[0];
    return "Speaker 1";
}
export function enforceSuggestionGuardrails(suggestions, input) {
    const warnings = [];
...
export function enforceComplianceGuardrails(issues, input) {
    const warnings = [];
...
export function enforceComposeGuardrails(output, input) {
    const warnings = [];
    ...
    return {
        output: {
            ...
            stages: output.stages.length === COMPOSE_STAGE_TEMPLATE.length
...
//# sourceMappingURL=aiGuardrailService.js.map