...
const DATE_PATTERN = /\b(?:\d{4}-\d{2}-\d{2}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{1,2},\s+\d{4})\b/gi;
function replaceWithCount(input, pattern, placeholder) {
    let count = 0;
...
function sanitizeUnknown(value, path, droppedKeyPaths, visited) {
...
export function deidentifyEncounterContext(input) {
