...
import type { AiTaskType, OrchestrationTrace, RunTaskInput, RunTaskOutput } from "./types.js"
...
const PAYLOAD_SCHEMAS: Record<AiTaskType, z.ZodTypeAny> = {
  suggestions: z
    .object({
      noteText: z.string(),
...
function buildPayloadStats(value: unknown) {
  let objectCount = 0
  let arrayCount = 0
  let stringCount = 0
  let stringChars = 0
...
async function callOpenAiJsonTask<TSchema extends z.ZodTypeAny>(input: {
  payload: unknown
  instructions: string
  schema: TSchema
  model?: string
