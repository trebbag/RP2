...
function toCentsDollars(cents: number): number {
...
function buildVendorPayload(payload: DispatchPayloadInput, vendor: DispatchContractVendor): DispatchContractResult {
  const idempotencyKey = payload.dispatchMetadata?.idempotencyKey || payload.encounterExternalId
  const contractVersion = payload.dispatchMetadata?.contractVersion || "v1"
  const common = {
...
export function buildDispatchContract(payload: DispatchPayloadInput): DispatchContractResult {
...
export function buildDispatchContractForConfig(
  payload: DispatchPayloadInput,
  config: { target: DispatchContractTarget; vendor?: DispatchContractVendor }
