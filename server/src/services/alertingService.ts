...
function normalizeSeverity(severity: AlertSeverity): "info" | "warning" | "error" {
  if (severity === "critical") return "error"
  return severity
}

function slackColor(severity: AlertSeverity): string {
  if (severity === "critical") return "#d00000"
  if (severity === "warning") return "#f59f00"
  return "#228be6"
}
...
async function sendPagerDutyAlert(input: AlertInput) {
...
async function sendWebhookAlert(input: AlertInput) {
...
export async function sendOperationalAlert(input: AlertInput) {
  ...
  if (env.ALERT_WEBHOOK_URL) sinks.push(sendWebhookAlert(input))
  if (env.ALERT_SLACK_WEBHOOK_URL) sinks.push(sendSlackAlert(input))
  if (env.PAGERDUTY_ROUTING_KEY) sinks.push(sendPagerDutyAlert(input))
