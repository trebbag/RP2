...
0. OIDC / SSO configuration (required when `AUTH_MODE=oidc`)

- `OIDC_ISSUER_URL`
- `OIDC_CLIENT_ID`
- `OIDC_CLIENT_SECRET`
- `OIDC_REDIRECT_URI` (must be registered with your IdP; typically `https://api.<domain>/api/auth/oidc/callback`)
- Confirm which claim should be treated as the canonical user email (default: `email`).
- Confirm whether we should auto-provision users into the default org or map org membership from IdP groups/claims.
